...
def main():
    latest_version, release_url = get_latest_version(api_url)
    app = QApplication([])
    if latest_version:
        if is_update_available(current_version, latest_version):
            message = f"New update available! Latest version: {latest_version}"
            reply = QMessageBox.question(None, "Update Available", f"{message}\nDo you want to open the release page?",
